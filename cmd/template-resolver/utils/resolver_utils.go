...
func processOperatorPolicyTemplates(
	...
	if found {
		...
		//nolint: errcheck // the nesting levels were already verified
		unstructured.SetNestedField(operatorPolicy, resolved, "spec", "operatorGroup")
	}
	...
	if found {
		...
		//nolint: errcheck // the nesting levels were already verified
		unstructured.SetNestedField(operatorPolicy, resolved, "spec", "subscription")
	} else {
